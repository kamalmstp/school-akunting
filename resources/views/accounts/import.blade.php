				...
				<div class="card mb-3">
					<div class="card-header">
						<div class="d-flex justify-content-between align-items-center">
							<h5 class="card-title">Impor Akun - {{ $school->name }}</h5>
						</div>
					</div>
					<div class="card-body">
                        <p>Unduh template Excel <a href="{{ route('accounts.download-template') }}?t={{ time() }}" class="btn btn-sm btn-info">di sini</a> dan isi sesuai format.</p>
                        ...
                        @endif
                        <form action="{{ auth()->user()->role == 'SuperAdmin' ? route('accounts.import', $school) : route('school-accounts.import', $school) }}" method="POST" enctype="multipart/form-data">
                        @csrf
